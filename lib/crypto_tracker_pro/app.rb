...
module CryptoTrackerPro
  class App
    def run
      ...
      analyze_btn = TkButton.new(root, text: "Анализировать")
      analyze_btn.pack(pady: 10)
      canvas = chart_drawer.build_canvas(root)
      output_text = TkText.new(root, width: 45, height: 14, font: "Courier 10", background: "white")
      output_text.pack(pady: 5)
      TkLabel.new(root, text: "Машина времени (вложили 100$ год назад):").pack(pady: 5)
      portfolio_text = TkText.new(root, width: 45, height: 3, font: "Courier 10", background: "white")
      portfolio_text.pack
